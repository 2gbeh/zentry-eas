...
if ($_SERVER['REQUEST_METHOD'] === 'POST')
{
	...
	$duplicate = sql_select($db_con, $tb, 'serial_no', $post['serial_no']);
	if (is_array($duplicate))
	{
								$error = 'Unique File No. already exist for a different record.';
								$errno = 400;				
	}
	else
	{
...
foreach ($db_res as $row)
{
	if ($row['STATUS'] < 1)
	{
		...
		for ($i = 1; $i <= $j; $i++) {
			...
			$td .= '<td>
				<label>Class '.$i.':</label>
				<input type="number" name="'.$name.'[]" min="0" value="'.alt_f($_POST[$name]).'" maxlength="3" />
			</td>';
		}
		$tr .= $td.'</tr>
				</table>
